...
onNotification(ServiceNotificationEvent event) async {
  ...
  if (recentCapturedNotifications.length > 50) {
    recentCapturedNotifications.removeRange(50, recentCapturedNotifications.length);
  }
...
Future queueTransactionFromMessage(String messageString,
  ...
  // We don't need this line, we can still queue it up a transaction without these details,
  ...
  if (entireAppLoaded == false || forceParse) {
    if (appStateSettings["AutoTransactions-canReadEmails"] == true) {
      ...
      if (newEmailCount > 0 || sayUpdates == true)
        openSnackbar(
          SnackbarMessage(
            title: "Scanned " + (results.messages?.length ?? 0).toString() + " emails",
